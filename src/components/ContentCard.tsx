...
export default function ContentCard({ zoneId, onClose, stats, onViewResume, onPlayAgain }: ContentCardProps) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      {/* Card */}
      <div className="relative bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto p-5 sm:p-8 animate-in">
        {/* Close button */}
        <button
          onClick={onClose}
    ...
    <>
      ...
      <div className="flex flex-wrap gap-3">
        <a
          ...
          className="flex items-center gap-2 px-4 py-2 bg-slate-800 text-white rounded-xl hover:bg-slate-700 transition-colors text-sm"
        >
        ...
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-xl hover:bg-blue-500 transition-colors text-sm"
        >
          <svg viewBox="0 0 24 24" className="w-5 h-5" fill="currentColor">
      ...
      {(onViewResume || onPlayAgain) && (
        <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 mt-6 pt-6 border-t border-slate-200">
          {onViewResume && (
            <button
